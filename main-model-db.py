...
class Pelicula(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    Nombre_pasajero = Column(String)
    Origen = Column(Text)
    Destino = Column(Integer)
    Fecha_vuelo= Column(Text)
    Reserva= Column(String)
# Crea la tabla si no existe
Base.metadata.create_all(bind=engine)
...
# Ruta para obtener todas las reseñas
@app.get("/reservavuelo/", response_model=list[ReviewOuts])
async def read_reviews(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    reviews = db.query(Review).offset(skip).limit(limit).all()
    return reviews

# Ruta para crear una reseña
@app.post("/reservavuelo/", response_model=ReviewOuts)
async def create_review(review: ReviewCreates, db: Session = Depends(get_db)):
    db_review = Review(**review.dict())
    db.add(db_review)
    db.commit()
    db.refresh(db_review)
...
# Ruta para obtener una reseña por ID
@app.get("/reservavuelo/{review_id}", response_model=ReviewOuts)
async def read_review(review_id: int, db: Session = Depends(get_db)):
    db_review = db.query(Review).filter(Review.id == review_id).first()
    if db_review is None:
        raise HTTPException(status_code=404, detail="Review not found")
...
# Ruta para actualizar una reseña por ID
@app.put("/reservavuelo/{review_id}", response_model=ReviewOuts)
async def update_review(review_id: int, review: ReviewUpdates, db: Session = Depends(get_db)):
    db_review = db.query(Review).filter(Review.id == review_id).first()
    if db_review is None:
        raise HTTPException(status_code=404, detail="Review not found")
    db_review.title = review.title
...
# Ruta para eliminar una reseña por ID
@app.delete("/reservavuelo/{review_id}")
async def delete_review(review_id: int, db: Session = Depends(get_db)):
    db_review = db.query(Review).filter(Review.id == review_id).first()
    if db_review is None:
        raise HTTPException(status_code=404, detail="Review not found")
